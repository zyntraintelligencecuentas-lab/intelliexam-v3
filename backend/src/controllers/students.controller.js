...
exports.create = async (req, res, next) => {
  try {
    const { name, list_number, curp, notes, initial_grade } = req.body;
    ...
    if (initial_grade !== undefined && initial_grade !== null && initial_grade !== '') {
      const examId = crypto.randomUUID();
      await turso.execute({
        sql: `INSERT INTO exams (id, teacher_id, title, subject, status) VALUES (?, ?, 'Diagnóstico Inicial', 'General', 'processed')`,
        args: [examId, req.user.id]
      });

      await turso.execute({
        sql: `INSERT INTO exam_results (id, exam_id, student_id, teacher_id, score) VALUES (?, ?, ?, ?, ?)`,
        args: [crypto.randomUUID(), examId, id, req.user.id, parseFloat(initial_grade)]
      });
    }
